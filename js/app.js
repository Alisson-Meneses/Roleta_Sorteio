...
(() => {
  ...
  /* ---------- hashing simples de senha (client-side only) ----------
     Não usa crypto.subtle de propósito: essa API só funciona em
     contexto seguro (https ou localhost) e falharia silenciosamente
     se o arquivo for aberto direto no navegador (file://). Como isso
     não é um cofre de senhas real (está tudo documentado no README),
     um hash simples e síncrono é suficiente e funciona em qualquer
     situação. */
  ...
  function randomSalt() {
    // Math.random é suficiente aqui: é só um sal para não repetir hashes
    // iguais entre usuários com a mesma senha, não uma chave criptográfica.
    let s = "";
  ...
  function handleLogin(e) {
    ...
    const hash = simpleHash(user.salt + password);
